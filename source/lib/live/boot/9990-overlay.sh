...
setup_unionfs ()
{
	image_directory="${1}"	# /live/medium/live/
	rootmnt="${2}"		# /root
	...
	then
		...
		do
			...
			then
				...
				mkdir -p "${mpoint}"
				#fstype = squashfs
				#image = /live/medium/live/filesystem.squashfs
				#backdev = /dev/loop0
	...
	echo "PERSISTENCE=${PERSISTENCE} NOPERSISTENCE=${NOPERSISTENCE}"
	...
	echo "EXPOSED_ROOT=${EXPOSED_ROOT}"
	if [ -n "${EXPOSED_ROOT}" ]
	then
		if [ ${rootfscount} -ne 1 ]
		then
	...
	for dir in ${cow_dirs}; do
		...
		then
			...
			if [ -n "${PERSISTENCE_READONLY}" ] && [ "${cowdevice}" != "tmpfs" ]
			then
				echo do_union ${unionmountpoint} ${cow_dir} ${root_backing} ${rootfs_dir}
				sleep 5
				#do_union /root/	    /	       /live/overlay/  //filesystem.squashfs/
				do_union ${unionmountpoint} ${cow_dir} ${root_backing} ${rootfs_dir}
			else
				echo do_union ${unionmountpoint} ${cow_dir} ${rootfs_dir}
				sleep 5
	...
	echo "rootfslist=${rootfslist}"		# //filesystem.squashfs
	live_rootfs_list=""
	for d in ${rootfslist}
	do
