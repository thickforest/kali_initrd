...
Main ()
{
	...
	then
		...
		then
			...
			while [ "$i" -lt 60 ]
			do
				livefs_root=$(find_livefs ${i}) # 返回find_livefs中的echo输出,并不是find_livefs最后的return返回值
				#/bin/sh
	...
	then
		...
		echo mount_images_in_directory "${livefs_root}" "${rootmnt}" "${mac}"
		sleep 5
